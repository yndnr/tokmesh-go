...
func TestNewReloadHandler(t *testing.T) {
	h := NewReloadHandler()
	if h == nil {
		t.Fatal("NewReloadHandler returned nil")
	}
	if h.hooks == nil {
		t.Error("hooks should be initialized")
	}
...
func TestNewReloadHandler_WithLogger(t *testing.T) {
	logger := &testLogger{}
	h := NewReloadHandler(WithReloadLogger(logger.Logger()))
	if h == nil {
		t.Fatal("NewReloadHandler returned nil")
	}
}

func TestReloadHandler_OnReload(t *testing.T) {
	h := NewReloadHandler()
	...
	h.mu.Lock()
	if len(h.hooks) != 2 {
		t.Errorf("expected 2 hooks, got %d", len(h.hooks))
	}
	h.mu.Unlock()
}

func TestReloadHandler_Reload(t *testing.T) {
	h := NewReloadHandler()
	...
	// Register hooks in order 1, 2, 3
	// They should be called in order: 1, 2, 3 (not reverse like shutdown)
	h.OnReload(func() error {
		mu.Lock()
		callOrder = append(callOrder, 1)
		mu.Unlock()
	...
	if len(callOrder) == 3 {
		if callOrder[0] != 1 || callOrder[1] != 2 || callOrder[2] != 3 {
			t.Errorf("hooks called in wrong order: %v, want [1, 2, 3]", callOrder)
		}
	}
}

func TestReloadHandler_Reload_HookError(t *testing.T) {
	h := NewReloadHandler()
	...
	// Even if one hook fails, all hooks should be called
	h.Reload()
...
func TestReloadHandler_StartStop(t *testing.T) {
	h := NewReloadHandler()
...
func TestReloadHandler_SIGHUP(t *testing.T) {
	h := NewReloadHandler()
	...
	if count != 1 {
		t.Errorf("expected 1 hook call after SIGHUP, got %d", count)
	}
...
func TestReloadHandler_MultipleSIGHUP(t *testing.T) {
	h := NewReloadHandler()
	...
	if count != 3 {
		t.Errorf("expected 3 hook calls after 3 SIGHUPs, got %d", count)
	}
...
func TestReloadHandler_ConcurrentOnReload(t *testing.T) {
	h := NewReloadHandler()
...
// testLogger is a helper for testing with custom logger
